...
class FuzzifikasiController extends Controller
{
    ...
    public function hasilFuzzifikasi(Request $request)
    {
        ...
        $result = app('App\Http\Controllers\DefuzzifikasiController')->hasilDefuzzifikasi();
        $inferensi = ModelInferensi::all();
        return view('defuzzifikasi', compact('hitungx_game', 'hitungx_office', 'hitungx_editing', 'hitungx_programming', 'inferensi'), $result);
        //$result=$this->hasilFuzzifikasi2();
    }

    public function hasilFuzzifikasi2()
    {
        //$this->hsl=$this->hasilFuzzifikasi();
        print_r($this->hsl);
    }
